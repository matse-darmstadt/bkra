...
public class Test_Cube {
	...
	public void testStartsegment() {
		...
		for (int x = 0; x < 3; x++)
			for (int y = 0; y < 3; y++)
				for (int z = 0; z < 3; z++) {
					...
					assertFalse("K�sesegment wurde als Startsegment gefunden",
							cube.isValidStartSegment(segments[x][y][z]));

					segments[x][y][z].setType(SegmentType.AIR);
					assertEquals("K�sesegment gefunden", SegmentType.AIR, segments[x][y][z].getType());
		...
		assertEquals("Die L�nge stimmt nicht", 3, solver.getPathLength());

		List<Segment> erwarteterWeg = Arrays.asList(segments[1][1][0], segments[1][1][1], segments[1][1][2]);
		List<Segment> weg = solver.getPath();
		assertEquals("Die L�nge stimmt nicht �berein", erwarteterWeg.size(), weg.size());
